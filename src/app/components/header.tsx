    ...
    <header className="border-b border-gray-200 sticky top-0 z-50 backdrop-blur-sm bg-white/95">
      <div className="max-w-6xl mx-auto px-4 md:px-6">
        ...
        <div className="py-3 md:py-5 flex items-center justify-between">
          <Link
            href="/"
            className={`flex items-center gap-2 md:gap-4 group transform transition-all duration-700 hover:scale-105 ${
          ...
          >
            ...
            <div className="overflow-hidden">
              ...
              <h1 className={`text-lg md:text-2xl font-black text-gray-900 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-yellow-600 group-hover:to-amber-600 transition-all duration-500 tracking-tight transform ${
              ...
              {/* Subtitle */}
              <div className={`text-xs text-gray-400 uppercase tracking-wider font-medium -mt-1 transition-all duration-700 transform group-hover:text-gray-600 ${
                isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
              }`}
              style={{ 
          ...
          <div className={`hidden md:flex items-center gap-6 transform transition-all duration-700 ${
            isVisible ? 'translate-x-0 opacity-100' : 'translate-x-10 opacity-0'
          }`}
            ...
            <div className="relative min-h-[3rem] flex items-center">
              <div className="w-full max-w-sm">
                {rotatingTexts.map((text, index) => (
                  <blockquote
                    key={index}
                    className={`text-gray-500 text-sm font-light italic transition-all duration-700 ease-in-out ${
                      index === textIndex 
          ...
          {/* Mobile-only dots indicator - แสดงเฉพาะใน mobile */}
          <div className={`md:hidden flex gap-1 transform transition-all duration-700 ${
            isVisible ? 'translate-x-0 opacity-100' : 'translate-x-10 opacity-0'
          }`}
          style={{ transitionDelay: '600ms' }}>
            {rotatingTexts.map((_, index) => (
